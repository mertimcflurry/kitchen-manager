...
export const load: PageServerLoad = () => {
	const { open, done } = listShopping(db);
	return { open, done, suggestions: shoppingSuggestions(db) };
};
...
export const actions: Actions = {
	...
	toggle: async ({ request }) => {
		...
		setShoppingDone(db, id, data.get('done') === 'true');
		return { ok: true };
	},
	...
	addProduct: async ({ request }) => {
		...
		addProductToList(db, productId, data.get('source') === 'suggested' ? 'suggested' : 'manual');
		return { ok: true };
	},

	/** Freitext aus dem Feld. Leeres bleibt folgenlos statt zu meckern. */
	addText: async ({ request }) => {
		const data = await request.formData();
		const text = String(data.get('text') ?? '');
		addTextToList(db, text);
		return { ok: true };
	},

	clearDone: async () => {
		clearDoneShopping(db);
		return { ok: true };
	}
};
