...
describe('recipeStock', () => {
	it('liefert nur offenen Bestand', () => {
		...
		const id = addStock(db, { productId, quantity: 400, unit: 'g', location: 'fridge' });

		const rows = recipeStock(db);
	...
	it('lässt aufgebrauchte Posten weg', () => {
		...
		const id = addStock(db, { productId, quantity: 1, unit: 'piece', location: 'fridge' });
		db.update(stockItem).set({ consumedAt: new Date() }).where(eq(stockItem.id, id)).run();

		expect(recipeStock(db).some((r) => r.id === id)).toBe(false);
	});
});
...
describe('cookIngredients', () => {
	it('zieht lose Ware um amountBase ab', () => {
		...
		const snapshots = cookIngredients(db, [{ stockItemId: id, amountBase: 200 }]);
	...
	it('klemmt lose Ware auf 0, statt ins Negative zu rechnen', () => {
		...
		const id = addStock(db, { productId, quantity: 150, unit: 'g', location: 'pantry' });

		const snapshots = cookIngredients(db, [{ stockItemId: id, amountBase: 500 }]);
	...
	it('rundet bei Stückware und klemmt gegen den Restbestand', () => {
		...
		const id = addStock(db, { productId, quantity: 3, unit: 'piece', location: 'fridge' });

		const snapshots = cookIngredients(db, [{ stockItemId: id, amountBase: 10.4 }]);
	...
	it('bucht bei angebrochener loser Ware gegen die Restmenge ab, nicht gegen die Kaufmenge', () => {
		...
		const id = addStock(db, { productId, quantity: 1000, unit: 'ml', location: 'fridge' });
	...
	it('reduziert bei angebrochener loser Ware nur den Füllstand, wenn Restmenge übrig bleibt', () => {
		...
		cookIngredients(db, [{ stockItemId: id, amountBase: 250 }]);
	...
	it('lässt eine verschlossene Packung stehen, wenn amountBase 0 ist', () => {
		...
		const id = addStock(db, { productId, quantity: 1, unit: 'pack', location: 'pantry' });

		const snapshots = cookIngredients(db, [{ stockItemId: id, amountBase: 0 }]);
	...
	it('lässt eine verschlossene Packung auch dann stehen, wenn amount_base fälschlich > 0 kommt', () => {
		...
		const id = addStock(db, { productId, quantity: 3, unit: 'pack', location: 'pantry' });
	...
	it('überspringt einen bereits aufgebrauchten Posten', () => {
		...
		const snapshots = cookIngredients(db, [{ stockItemId: id, amountBase: 1 }]);
		expect(snapshots).toEqual([]);
	});
	...
	it('lässt eine zweite, nicht referenzierte Charge desselben Produkts unangetastet', () => {
		...
		const referenced = addStock(db, { productId, quantity: 1, unit: 'pack', location: 'fridge' });
		const otherBatch = addStock(db, { productId, quantity: 1, unit: 'pack', location: 'fridge' });

		const snapshots = cookIngredients(db, [{ stockItemId: referenced, amountBase: 0 }]);
	...
	it('überspringt eine ungültige stock_item_id, ohne die übrigen Zutaten zu blockieren', () => {
		const productId = findOrCreateProduct(db, 'Milch');
		const id = addStock(db, { productId, quantity: 500, unit: 'ml', location: 'fridge' });

		const snapshots = cookIngredients(db, [
...
describe('undoCookIngredients', () => {
	it('stellt die Menge vor dem Abbuchen wieder her', () => {
		...
		const id = addStock(db, { productId, quantity: 500, unit: 'g', location: 'pantry' });
